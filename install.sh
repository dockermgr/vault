...
HUB_IMAGE_URL="casjaysdevdocker/vault"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
CONTAINER_WEB_SERVER_VHOSTS="vault.all"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
DOMAIN=$PROXY_HTTP_PROTO://$CONTAINER_HOSTNAME
...
ADMIN_TOKEN="$CONTAINER_USER_ADMIN_PASS_HASH"
ADMIN_SESSION_LIFETIME=3600
DATABASE_URL=${DATABASE_DIR_SQLITE3:-$DATABASE_BASE_DIR}/$APPNAME.sqlite3
PUSH_ENABLED=true
PUSH_INSTALLATION_ID="f1d959bd-2a6d-474e-ab1d-b0560046e6e6"
PUSH_INSTALLATION_KEY="5tzNCYUNwRyy37Zj7KRK"
#YUBICO_CLIENT_ID=
#YUBICO_SECRET_KEY=
#DUO_IKEY=
#DUO_SKEY=
#SMTP_HOST=${CONTAINER_EMAIL_RELAY:-$SET_DOCKER_IP}
#SMTP_FROM=${CONTAINER_EMAIL_USER:-no-reply@}${CONTAINER_EMAIL_DOMAIN:-$CONTAINER_HOSTNAME}
#SMTP_PORT=587
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set custom script - saves to /config/env/vault.script.sh
...
CONTAINER_HOSTNAME="${CONTAINER_HOSTNAME:-${APPNAME:-vault}}"
...
if [ -n "$VAULT_USERNAME" ]; then
  CONTAINER_USER_NAME="$VAULT_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${VAULT_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
